...
const css = `
  .ai-container { font-family: 'DM Sans', sans-serif; color: #e8e4dc; display: flex; align-items: center; gap: 16px; }
  .ai-container * { box-sizing: border-box; }
  .ai-header {
    display: flex; flex-direction: column; gap: 6px; flex-shrink: 0;
    width: 200px; padding-right: 14px; border-right: 0.5px solid #2a2a30;
  }
  ...
  .ai-heading-icon {
    ...
    font-size: 11px; color: #0f0f11; font-weight: 700; flex-shrink: 0;
  }
  .ai-title { font-family: 'DM Serif Display', serif; font-size: 1rem; margin: 0; color: #e8e4dc; letter-spacing: -0.3px; line-height: 1.1; }
  .ai-subtitle { font-family: 'DM Mono', monospace; font-size: 0.58rem; letter-spacing: 0.12em; text-transform: uppercase; color: #555; }
  ...
  .ai-btn-primary {
    ...
    padding: 6px 12px; border-radius: 5px; font-family: 'DM Sans', sans-serif;
  ...
  .ai-spinner {
    width: 10px; height: 10px; border: 1.5px solid #0f0f11;
  ...
  .ai-content {
    flex: 1; min-width: 0;
    background: #0f0f11; border: 0.5px solid #222226; border-radius: 6px;
    padding: 10px 14px; max-height: 92px; overflow-y: auto;
  ...
  .ai-content h2, .ai-content h3, .ai-content h4 {
    ...
    font-size: 0.75rem; font-weight: 600; margin: 8px 0 3px;
  }
  .ai-content h2:first-child, .ai-content h3:first-child, .ai-content h4:first-child { margin-top: 0; }
  .ai-content p { margin: 0 0 4px; color: #c8c4bc; }
  .ai-content p:last-child { margin-bottom: 0; }
  .ai-content ul, .ai-content ol { margin: 2px 0 0; padding-left: 18px; color: #c8c4bc; }
  ...
  .ai-content code {
    background: #1e1e22; padding: 1px 4px; border-radius: 3px;
    font-family: 'DM Mono', monospace; font-size: 0.72rem; color: #e8b84b;
  }
  ...
  .ai-empty { color: #555; }
  .ai-loading { color: #888; display: flex; align-items: center; gap: 8px; }
  .ai-loading-dots {
    width: 11px; height: 11px; border: 1.5px solid #e8b84b;
    border-top-color: transparent; border-radius: 50%;
    animation: ai-spin 0.8s linear infinite;
  }
  .ai-error { color: #e05a5a; text-align: left; }
  .ai-error strong { color: #e05a5a; }

  @media (max-width: 900px) {
    .ai-container { flex-direction: column; align-items: stretch; gap: 10px; }
    .ai-header { width: auto; padding: 0 0 10px; border-right: none; border-bottom: 0.5px solid #2a2a30; flex-direction: row; justify-content: space-between; align-items: center; }
    .ai-content { max-height: 130px; }
  }
    ...
    <div class="ai-header">
      <div class="ai-heading">
        ...
        <div>
          ...
          <div class="ai-subtitle" id="ai-status">OpenAI</div>
        </div>
      </div>
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are a project management assistant for the TaskLU dashboard. Output ONLY: one short sentence summarizing project status, then up to 3 markdown bullets of the most important next actions. No headers, no preamble, no closing. Total under 60 words. Reference actual task titles. Never invent tasks.',
        },
        {
          role: 'user',
...
export function initAiOverview(containerId) {
  ...
  const refreshStatus = () => {
    ...
    const parts = ['OpenAI', hasKey ? 'key set' : 'no key'];
    try {
      const cached = JSON.parse(localStorage.getItem(CACHE_KEY) || 'null');
      if (cached?.at) parts.push(formatTimestamp(cached.at));
    } catch { /* ignore */ }
